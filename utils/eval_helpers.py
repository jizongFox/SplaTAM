...
def report_loss(losses, wandb_run, wandb_step, tracking=False, mapping=False):
    ...
    if tracking:
        ...
        frame_opt_loss_dict[
            "Per Iteration Current Frame Optimization/step"
        ] = wandb_step
...
def eval_online(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if mapping_iters == 0 and not add_new_gaussians:
            psnr = calc_psnr(
                im * presence_sil_mask, curr_data["im"] * presence_sil_mask
            ).mean()
        else:
    ...
    plt.savefig(
        os.path.join(eval_online_dir, "online_metrics.png"), bbox_inches="tight"
    )
    if wandb_run is not None:
...
def eval_nvs(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if transform_rots:
            ...
            transformed_gaussians["unnorm_rotations"] = final_params[
                "unnorm_rotations"
            ].detach()
        ...
        if save_frames:
            ...
            cv2.imwrite(
                os.path.join(
                    render_rgb_dir, "splatam_{:04d}.png".format(test_time_idx)
                ),
                cv2.cvtColor(viz_render_im * 255, cv2.COLOR_RGB2BGR),
            )
            cv2.imwrite(
                os.path.join(
                    render_depth_dir, "splatam_{:04d}.png".format(test_time_idx)
                ),
                depth_colormap,
            )
            ...
            cv2.imwrite(
                os.path.join(depth_dir, "gt_{:04d}.png".format(test_time_idx)),
                depth_colormap,
            )
    ...
    axs[1].set_ylabel("L1 (cm)")
    fig.suptitle(
        "Average PSNR: {:.2f}, Average Depth L1: {:.2f} cm".format(
            avg_psnr, avg_l1 * 100
        ),
        y=1.05,
        fontsize=16,
